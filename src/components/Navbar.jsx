...
import { Bell, User, Search, X, Menu } from "lucide-react"; // ✅ Added Menu icon for Hamburger
...
function Navbar({ jobs, userInfo, setSidebarOpen }) {
  ...
  const appliedJobs = jobs.filter((job) => job.status.toLowerCase() === "applied");
  const interviewJobs = jobs.filter((job) => job.status.toLowerCase() === "interview");
  const rejectedJobs = jobs.filter((job) => job.status.toLowerCase() === "rejected");
    ...
    <>
        ...
        <div className="flex items-center gap-2">
          {isMobile && (
            <Menu
              ...
              onClick={() => setSidebarOpen((prev) => !prev)} // ✅ Toggles sidebar
            />
          )}
      ...
      {showProfile && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className={`bg-white p-6 rounded-lg shadow-lg ${isMobile ? "w-[90%]" : "w-[400px]"} relative`}>
            {/* Modal Header */}
            ...
            <div className="bg-gray-100 p-3 rounded-lg mb-4">
              <h3 className="text-lg font-semibold text-gray-800">Applications Summary</h3>
            ...
            <div className="max-h-48 overflow-y-auto">
              ...
              {jobs.length > 0 ? (
                jobs.map((job) => (
                  <div key={job.id} className="border p-3 rounded-md mb-2 bg-gray-50">
                    <p className="text-gray-800"><strong>Company:</strong> {job.company}</p>
                    <p className="text-gray-800"><strong>Title:</strong> {job.title}</p>
                    <p className="text-gray-700"><strong>Status:</strong> {job.status}</p>
                    <p className="text-gray-600 text-sm"><strong>Date:</strong> {job.date}</p>
                  </div>
                ))
              ) : (
